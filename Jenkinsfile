...
def archiveAndCleanup() {
    dir (env.WORKSPACE) {
        archiveArtifacts onlyIfSuccessful: false, allowEmptyArchive: true, artifacts: 'functional-tests/target/test-output/**/*,functional-tests/target/surefire-reports/**/*,functional-tests/target/failsafe-reports/**/*,_wsb/functional-tests/target/test-output/**/*,_wsb/functional-tests/target/surefire-reports/**/*,_wsb/functional-tests/target/failsafe-reports/**/*'
    }
    sh '''
            ...
            stages{
                ...
                stage('copy-workspace') {
                    steps {
                        sh '''
                            rm -rf _wsb
                            mkdir _wsb
                            find . -type d -not -name _wsb -not -name . -maxdepth 1 -exec cp -R \\{} _wsb \\;
                            find . -type f -maxdepth 1 -exec cp \\{} _wsb \\;
                        '''
                    }
                }
                stage('kubernetes-tests-parallel') {
                    parallel {
                        ...
                        stage('test-B') {
                            steps {
                                testStep('./_wsb', env.BUILD_NUMBER + '-2', '-P testLatestCoherenceReleasedImage')
                            }
                        }
                    }
                }
